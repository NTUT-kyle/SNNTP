...
(function ($) {
    $.fn.connect = function (param, posX, posY) {
        ...
        /* extend delete method by kyL */
        this.deleteLine = function (option) {
            let findNode = -1;
            for (let i = 0; i < _lines.length; i++) {
                if (
                    _lines.at(i).left_node == option.left_node &&
                    _lines.at(i).right_node == option.right_node
                ) {
                    findNode = i;
                    break;
                }
        ...
        this.drawAllLine = function (option) {
			...
			*/
            ...
            if (
                option.left_selector != "" &&
                typeof option.left_selector !== "undefined" &&
                $(option.left_selector).length > 0
            ) {
                $(option.left_selector).each(function (index) {
                    ...
                    if (
                        option2.right_node != "" &&
                        typeof option2.right_node !== "undefined"
                    ) {
        ...
        this.connect = function (option) {
            ...
            try {
				...
				*/
                ...
                if (
                    option.left_node != "" &&
                    typeof option.left_node !== "undefined" &&
                    option.right_node != "" &&
                    typeof option.right_node !== "undefined" &&
                    $(option.left_node).length > 0 &&
                    $(option.right_node).length > 0
                ) {
                    ...
                    $(option.right_node).each(function (index, value) {
                        ...
                        _right_node = $(value);
                        if (
                            _left_node.offset().left >=
                            _right_node.offset().left
                        ) {
                            _tmp = _left_node;
                        ...
                        _left.x =
                            _left_node.offset().left +
                            _left_node.outerWidth() -
                            residualWidth;
                        _left.y =
                            _left_node.offset().top +
                            _left_node.outerHeight() / 2 -
                            residualHeight;
                        _right.x = _right_node.offset().left - residualWidth;
                        _right.y =
                            _right_node.offset().top +
                            _right_node.outerHeight() / 2 -
                            residualHeight;
                    ...
                    if (_error)
                        alert("Mandatory Fields are missing or incorrect");
                }
            } catch (err) {
